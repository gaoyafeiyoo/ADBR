...
def train_wanet(args, model, train_loader, criterion, optimizer, noise_grid, identity_grid, epoch):
    ...
    for batch_idx, (x, y) in enumerate(train_loader):
        ...
        if args.pt:
            ...
            total_inputs = torch.cat([inputs_bd, inputs_cross, x[(num_bd + num_cross):]], dim=0).to(args.device)
        ...
        try:
            total_cross_correct += torch.sum(
                torch.argmax(total_preds[num_bd: (num_bd + num_cross)], dim=1) == total_targets[
                                                                        num_bd: (num_bd + num_cross)])
        except:
            pass
    ...
    print()
    to_log_file(
        'Epoch %3d: Loss %.3f Clean acc %.2f BD asr %.2f Cross acc %.2f '
          % (epoch, avg_loss_ce, avg_acc_clean, avg_acc_bd, avg_acc_cross),
